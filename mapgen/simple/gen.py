...
class Cell:
	def __init__(self,tile_map,x,y):
		...
		self.connect = [isConnected(i) for i in range(4)]
...
class TileMap:
	...
	def reset(self):
		self.tiles = [[0 for i in range(mapwidth)] for j in range(mapheight)]
		self.setTile(0,3,1)
		self.setTile(1,3,1)
		self.setTile(0,4,1)
		self.setTile(1,4,1)
	...
	def buildCells(self):
		...
		self.cells = [[Cell(self,x,y) for x in range(mapwidth)] for y in range(mapheight)]
	...
	def getNextOpenTile(self,x,y):
		for x0 in range(x,mapwidth-1):
			for y0 in range(y,mapheight):
...
def genMapForAllRoots():
	...
	for i in range(numRoots):
		...
		print("%d / %d:" % (i+1,numRoots), file=sys.stderr)
